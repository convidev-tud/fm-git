...
impl CommandInterface for HiddenCompletionCommand {
    fn run_command(&self, args: &ArgMatches, state: &mut CommandContext) {
        ...
        let maybe_last_child = state
            .command_map
            .find_last_child_recursive(&mut to_complete);
        let last_item = if to_complete.is_empty() {
            None
        } else {
            Some(<&str>::clone(to_complete.last().unwrap()))
        };
                ...
                if last_item.is_some() {
                    ...
                    for subcommand in subcommands {
                        if subcommand != "__completion" {
                            state.log_to_stdout(subcommand.to_string())
                        }
                    }
                        ...
                        for arg in all_args {
                            if !unwrapped_last_item.starts_with("--") {
                                ...
                                if short.is_some() {
                                    ...
                                    if s.starts_with(unwrapped_last_item) {
